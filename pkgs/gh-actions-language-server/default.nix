{
  ...
  buildNpmPackage,
  bun,
  # Injected by overlays/default.nix from the "gh-actions-language-server"
  # flake input, which is fetched with flake = false.
  gh-actions-language-server-src,
}:
buildNpmPackage {
  pname = "gh-actions-language-server";
  version = "0.3.13";
  src = gh-actions-language-server-src;
